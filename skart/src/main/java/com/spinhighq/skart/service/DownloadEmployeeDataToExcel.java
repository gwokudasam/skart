...
import com.spinhighq.skart.Excel.EmployeeFillManager;
import com.spinhighq.skart.Excel.EmployeeLayouter;
import com.spinhighq.skart.Excel.EmployeeWriter;
import com.spinhighq.skart.model.EmployeeTable;

@Service("downloadEmployee")
@Transactional
public class DownloadEmployeeDataToExcel {
	@Autowired
	...
	public void DownloadXLS(HttpServletResponse response)
	 ...
	 // 6. Set the response properties
		...
		String fileName = schoolName+"_EmployeeTable_" + timeStamp + ".xls";
		...
		EmployeeWriter.write(response, worksheet);
	}
	...
	// @Transactional
	private List<EmployeeTable> getEmployeeData() {
		...
		Query query = session.createQuery("from EmployeeTable");
		// Execute query
		List<EmployeeTable> list = query.list();
		// Return the list
		return list;
